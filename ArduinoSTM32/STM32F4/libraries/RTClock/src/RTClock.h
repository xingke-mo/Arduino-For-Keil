...
public:
    ...
    void begin(rtc_clk_src _src)
    {
        begin(_src, 0, 0);
    }
    void begin()
    {
        begin(RTCSEL_LSE, 0, 0);
    }
...
#define breakTime _breakTime
    ...
    {
        return _makeTime(&_tm);
    }
    void setTime(time_t time_stamp)
    {
        breakTime(time_stamp, &_tm);
        _setTime(&_tm);
    }
    ...
    void now(tm_t & tmm)
    {
        _getTime(&tmm);    // non-standard use of now() function, added for compatibility with previous versions of the library
    }
    ...
    inline void attachSecondsInterrupt(voidFuncPtr function)
    {
        attachPeriodicWakeupInterrupt(function);
    }
    inline void detachSecondsInterrupt()
    {
        detachPeriodicWakeupInterrupt();
    }
