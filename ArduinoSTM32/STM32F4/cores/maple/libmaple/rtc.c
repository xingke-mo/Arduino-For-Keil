...
{
    return bcd2bin((dr >> RTC_DR_YEAR_BIT) & RTC_DR_YEAR_MASK);
}
uint8_t _month(uint32_t dr)
{
    return bcd2bin((dr >> RTC_DR_MONTH_BIT) & RTC_DR_MONTH_MASK);
}
uint8_t _day(uint32_t dr)
{
    return bcd2bin((dr >> RTC_DR_DAY_BIT) & RTC_DR_DAY_MASK);
}
uint8_t _weekday(uint32_t dr)
{
    return bcd2bin((dr >> RTC_DR_WEEKDAY_BIT) & RTC_DR_WEEKDAY_MASK);
}
uint8_t _pm(uint32_t tr)
{
    return ((tr >> RTC_TR_PM_BIT) & RTC_TR_PM_MASK);
}
uint8_t _hour(uint32_t tr)
{
    return bcd2bin((tr >> RTC_TR_HOUR_BIT) & RTC_TR_HOUR_MASK);
}
uint8_t _minute(uint32_t tr)
{
    return bcd2bin((tr >> RTC_TR_MINUTE_BIT) & RTC_TR_MINUTE_MASK);
}
uint8_t _second(uint32_t tr)
{
    return bcd2bin((tr >> RTC_TR_SECOND_BIT) & RTC_TR_SECOND_MASK);
}
