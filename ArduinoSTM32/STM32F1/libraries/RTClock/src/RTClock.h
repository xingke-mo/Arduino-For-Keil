...
static inline uint8_t bin2bcd(uint8_t b)
{
    return (((b / 10) << 4) + (b % 10));
}
...
public:
    ...
    void now(tm_t & tmm)
    {
        getTime(tmm);    // non-standard use of now() function, added for compatibility with previous versions of the library
    }
    ...
    {
        return (t + (HTZ * SECS_PER_HOUR) + (MTZ * 60));    // HTZ = Hour offset, MTZ = Minute offset
    }
