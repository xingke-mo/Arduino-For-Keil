...
protected:
    bool isMaster() const
    {
        return ((dev_flags & I2C_SLAVE_MODE) == 0);
    }
...
public:
    ...
    {
        return frequency;    // Get Master clock speed
    }
    ...
    uint16_t recvSlaveAddress() const
    {
        return itc_slave_msg.addr;
    }
    ...
    {
        if(sel_hard->state == I2C_STATE_DISABLED)
        {
            useGeneralCall = bEnable;    // Only allow change when disabled
        }
    }
    ...
    bool getGeneralCall() const
    {
        return useGeneralCall;
    }
